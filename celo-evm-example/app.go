...
func Run() error {
	...
	err = celo1Client.Configurate(viper.GetString(config.ConfigFlagName), "./config/chain/config_celo.json")
	if err != nil {
		panic(err)
	}
	...
	err = evmClient.Configurate(viper.GetString(config.ConfigFlagName), "./config/chain/config_evm.json")
	if err != nil {
		panic(err)
	}
